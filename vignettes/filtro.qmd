...
For a single score, use built-in methods:  
...
In `show_best_score_prop()`, there is a argument called `prop_terms` that lets us control the proportion of predictors to keep.
...
```{r}
# Fill safe value, then show best
...
Analogous to [`show_best_desirability()`](https://desirability2.tidymodels.org/reference/show_best_desirability.html), we can jointly optimize multiple scores. 
