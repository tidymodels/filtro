...
Recall earlier that a safe value can be used to retain the predictor. Here we show examples, but note that the safe value differs from the individual score object: 

```{r}
ames_aov_pval_natrual_res@fallback_value
ames_cor_pearson_res@fallback_value
ames_imp_rf_reg_res@fallback_value
ames_info_gain_reg_res@fallback_value
```
...
Other than `maximize()`, other verbs that are available are: `minimize()`, `target()`, and `constrain()`. 
