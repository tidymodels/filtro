...
- Since the outcome is numeric, any predictor that is not a factor will result in `NA`. 

- In case where `NA` is produced, a safe value (can be accessed via `object@fallback_value`) can be used to retain the predictor. 

- By default, it computes `-log10(p_value)`, so that larger values indicate more important predictors.
